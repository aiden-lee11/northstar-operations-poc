...
- Run the built local demo: `python3 app.py`
- Run on another local port: `python3 app.py --port 8080`
...
- Run all Python tests: `python3 -m unittest discover -v`
- Compile Python: `python3 -m py_compile app.py refunds.py flags.py test_app.py test_refunds.py test_flags.py`
- Check dependency advisories: `npm audit`

For local development, run `npm run dev` in one terminal and `python3 app.py` in another. The Vite workflow rebuilds `dist/`; Python remains the only HTTP server needed for the demo and serves the frontend and API together at `http://127.0.0.1:8000`.
...
- `app.py` owns the HTTP server, constrained `dist/` static serving, API routing, JSON validation, mutation request protections, and error mapping.
- `refunds.py` owns the read-only synthetic refund domain and summary.
- `flags.py` owns environment-isolated in-memory flag state, validation, optimistic concurrency, rollback, and audit events.
- `src/App.tsx`, `src/RefundsView.tsx`, `src/FlagsView.tsx`, and `src/SharedDialog.tsx` own the React component architecture and declarative interaction.
- `src/api.ts` and `src/models.ts` own the typed same-origin API client and frontend API models.
- `index.html` is the Vite entry document and `styles.css` remains the shared presentation source.
- `app.js` is an unused legacy reference. Do not load it, serve it, or treat it as active frontend source.
- `src/App.test.tsx` owns React behavior coverage. `test_app.py` owns HTTP integration coverage. `test_refunds.py` and `test_flags.py` own domain contracts.
